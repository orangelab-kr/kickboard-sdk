...
class KickboardClient {
    ...
    async start() {
        ...
        return (await this.waitForResponse(__1.KickboardStart(), 'status', match));
    }
    /** 킥보드를 종료합니다. */
    async stop() {
        const match = (packet) => packet.isEnabled === false;
        return (await this.waitForResponse(__1.KickboardStop(), 'status', match));
    }
    /** 킥보드를 잠금합니다. */
    lock() {
        this.sendMessage(__1.KickboardLock());
    }
    /** 킥보드를 잠금 해제합니다. */
    unlock() {
        this.sendMessage(__1.KickboardUnlock());
    }
    /** 배터리 잠금합니다. */
    batteryLock() {
        this.sendMessage(__1.BatteryLock());
    }
    /** 배터리 잠금을 해제합니다. */
    batteryUnlock() {
        this.sendMessage(__1.BatteryUnlock());
    }
    /** 부저를 킵니다. */
    async buzzerOn(mode, seconds = 0) {
        const match = (packet) => packet.isBuzzerOn === true;
        await this.waitForResponse(__1.BuzzerOn(mode, seconds), 'status', match);
    }
    /** 부저를 끕니다. */
    async buzzerOff() {
        const match = (packet) => packet.isBuzzerOn === false;
        await this.waitForResponse(__1.BuzzerOff(), 'status', match);
    }
    /** 블루투스를 킵니다. */
    bluetoothOn() {
        this.sendMessage(__1.BluetoothOn());
    }
    /** 블루투스를 끕니다. */
    bluetoothOff() {
        this.sendMessage(__1.BluetoothOff());
    }
    /** 불을 킵니다. */
    ...
    async getInfo() {
        return await this.waitForResponse(__1.KickboardInfo(), 'info');
    }
    /** GPS 정보, 속도와 같은 현재 상태를 불러옵니다. */
    async getStatus() {
        return (await this.waitForResponse(__1.KickboardStatus(), 'status'));
    }
    /** MQTT 와 GPRS 크레덴셜 및 기타 정보를 불러옵니다. */
    async getConfig() {
        return await this.waitForResponse(__1.ConfigGet(), 'config');
    }
    /** 배터리 정보와 상태를 가져옵니다. */
    async getBattery() {
        return (await this.waitForResponse(__1.BatteryStatus(), 'battery'));
    }
    /** 알람 모드를 켭니다. */
    alarmOn(mode, seconds = 0) {
        this.sendMessage(__1.AlarmOn(mode, seconds));
    }
    /** 알람 모드를 끕니다. */
    alarmOff() {
        this.sendMessage(__1.AlarmOff());
    }
    /** GPRS APAD를 수정합니다. */
    async setGprsApad(apad) {
        return (await this.waitForResponse(__1.ConfigSet('apad', apad), 'config'));
    }
    /** GPRS USERNAME를 수정합니다. */
    async setGrpsUsername(username) {
        return (await this.waitForResponse(__1.ConfigSet('auser', username), 'config'));
    }
    /** GPRS PASSWORD를 수정합니다. */
    async setGrpsPassword(password) {
        return (await this.waitForResponse(__1.ConfigSet('apass', password), 'config'));
    }
    /** MQTT Address 수정합니다. */
    async setMQTTAddress(address) {
        return (await this.waitForResponse(__1.ConfigSet('ip', address), 'config'));
    }
    /** MQTT Port 수정합니다. */
    async setMQTTPort(port) {
        return (await this.waitForResponse(__1.ConfigSet('port', `${port}`), 'config'));
    }
    /** Report Interval Ping을 수정합니다. */
    async setReportIntervalPing(seconds) {
        return (await this.waitForResponse(__1.ConfigSet('ping', `${seconds}`), 'config'));
    }
    /** Report Interval Trip을 수정합니다. */
    async setReportIntervalTrip(seconds) {
        return (await this.waitForResponse(__1.ConfigSet('tripint', `${seconds}`), 'config'));
    }
    /** Report Interval Static을 수정합니다. */
    async setReportIntervalStatic(seconds) {
        return (await this.waitForResponse(__1.ConfigSet('statint', `${seconds}`), 'config'));
    }
    /** MQTT Client ID를 수정합니다. */
    async setMQTTClientId(clientId) {
        return (await this.waitForResponse(__1.ConfigSet('mqid', clientId), 'config'));
    }
    /** MQTT Username을 수정합니다. */
    async setMQTTUsername(username) {
        return (await this.waitForResponse(__1.ConfigSet('mquser', username), 'config'));
    }
    /** MQTT Password을 수정합니다. */
    async setMQTTPassword(password) {
        return (await this.waitForResponse(__1.ConfigSet('mqpass', password), 'config'));
    }
    /** 속도를 수정합니다. */
    async setSpeedLimit(speed) {
        return (await this.waitForResponse(__1.ConfigSet('speedlim', `${speed}`), 'config'));
    }
    /** 충격 기준치를 수정합니다. */
    async setImpact(impact) {
        return (await this.waitForResponse(__1.ConfigSet('impact', `${impact}`), 'config'));
    }
    /** 블루투스 인증 키를 변경합니다. */
    async setBluetoothKey(key) {
        return (await this.waitForResponse(__1.ConfigSet('blekey', key), 'config'));
    }
    /** 네트워크 모드를 변경합니다. */
    async setNetworkMode(mode) {
        return (await this.waitForResponse(__1.ConfigSet('netconfig', `${mode}`), 'config'));
    }
    /** 킥보드를 재부팅합니다. */
    reboot() {
        this.sendMessage(__1.KickboardReboot());
    }
