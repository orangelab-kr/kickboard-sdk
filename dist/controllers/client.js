...
class KickboardClient {
    ...
    async start() {
        ...
        return (await this.waitForResponse(kickboard_1.KickboardStart(), 'status', match));
    }
    /** 킥보드를 종료합니다. */
    async stop() {
        const match = (packet) => packet.vehicle.isEnabled === false;
        return (await this.waitForResponse(kickboard_1.KickboardStop(), 'status', match));
    }
    /** 킥보드를 잠금합니다. */
    lock() {
    ...
    async buzzerOn(mode, seconds = 0) {
        ...
        await this.waitForResponse(buzzer_1.BuzzerOn(mode, seconds), 'status', match);
    }
    /** 부저를 끕니다. */
    async buzzerOff() {
        const match = (packet) => packet.isBuzzerOn === false;
        await this.waitForResponse(buzzer_1.BuzzerOff(), 'status', match);
    }
    /** 블루투스를 킵니다. */
    bluetoothOn() {
    ...
    async lightOn(mode, seconds = 0) {
        ...
        await this.waitForResponse(light_1.LightOn(mode, seconds), 'status', match);
    }
    /** 불을 끕니다. */
    async lightOff() {
        const match = (packet) => packet.isLightsOn === false;
        await this.waitForResponse(light_1.LightOff(), 'status', match);
    }
    /** 하드웨어, 소프트웨어 정보를 가져옵니다. */
    async getInfo() {
        return await this.waitForResponse(kickboard_1.KickboardInfo(), 'info');
    }
    /** GPS 정보, 속도와 같은 현재 상태를 불러옵니다. */
    async getStatus() {
        return (await this.waitForResponse(kickboard_1.KickboardStatus(), 'status'));
    }
    /** MQTT 와 GPRS 크레덴셜 및 기타 정보를 불러옵니다. */
    async getConfig() {
        return await this.waitForResponse(config_1.ConfigGet(), 'config');
    }
    /** 배터리 정보와 상태를 가져옵니다. */
    async getBattery() {
        return (await this.waitForResponse(battery_1.BatteryStatus(), 'battery'));
    }
    /** 알람 모드를 켭니다. */
    ...
    async setGprsApad(apad) {
        return (await this.waitForResponse(config_1.ConfigSet('apad', apad), 'config'));
    }
    /** GPRS USERNAME를 수정합니다. */
    async setGrpsUsername(username) {
        return (await this.waitForResponse(config_1.ConfigSet('auser', username), 'config'));
    }
    /** GPRS PASSWORD를 수정합니다. */
    async setGrpsPassword(password) {
        return (await this.waitForResponse(config_1.ConfigSet('apass', password), 'config'));
    }
    /** MQTT Address 수정합니다. */
    async setMQTTAddress(address) {
        return (await this.waitForResponse(config_1.ConfigSet('ip', address), 'config'));
    }
    /** MQTT Port 수정합니다. */
    async setMQTTPort(port) {
        return (await this.waitForResponse(config_1.ConfigSet('port', `${port}`), 'config'));
    }
    /** Report Interval Ping을 수정합니다. */
    async setReportIntervalPing(seconds) {
        return (await this.waitForResponse(config_1.ConfigSet('ping', `${seconds}`), 'config'));
    }
    /** Report Interval Trip을 수정합니다. */
    async setReportIntervalTrip(seconds) {
        return (await this.waitForResponse(config_1.ConfigSet('tripint', `${seconds}`), 'config'));
    }
    /** Report Interval Static을 수정합니다. */
    async setReportIntervalStatic(seconds) {
        return (await this.waitForResponse(config_1.ConfigSet('statint', `${seconds}`), 'config'));
    }
    /** MQTT Client ID를 수정합니다. */
    async setMQTTClientId(clientId) {
        return (await this.waitForResponse(config_1.ConfigSet('mqid', clientId), 'config'));
    }
    /** MQTT Username을 수정합니다. */
    async setMQTTUsername(username) {
        return (await this.waitForResponse(config_1.ConfigSet('mquser', username), 'config'));
    }
    /** MQTT Password을 수정합니다. */
    async setMQTTPassword(password) {
        return (await this.waitForResponse(config_1.ConfigSet('mqpass', password), 'config'));
    }
    /** 속도를 수정합니다. */
    async setSpeedLimit(speed) {
        return (await this.waitForResponse(config_1.ConfigSet('speedlim', `${speed}`), 'config'));
    }
    /** 충격 기준치를 수정합니다. */
    async setImpact(impact) {
        return (await this.waitForResponse(config_1.ConfigSet('impact', `${impact}`), 'config'));
    }
    /** 블루투스 인증 키를 변경합니다. */
    async setBluetoothKey(key) {
        return (await this.waitForResponse(config_1.ConfigSet('blekey', key), 'config'));
    }
    /** 네트워크 모드를 변경합니다. */
